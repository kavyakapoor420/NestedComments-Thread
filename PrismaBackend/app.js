...
// prisma+postgres://accelerate.prisma-data.net/?api_key=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhcGlfa2V5IjoiMDFKWkJEU0JCWTE2REFRUDVXSzFRSjYwQlQiLCJ0ZW5hbnRfaWQiOiI2MTJhMmRkMGE0ODc0OTFhOWI0MDFjYTdlZDUzNWZiYjUyMTEwNTUwNDAxMTFlMjkzNWE1OWZjNGY5YmM5NDcwIiwiaW50ZXJuYWxfc2VjcmV0IjoiMWVhZDIxZTQtNzM3Mi00MDAwLWIwOTQtY2NjODdhZWZmMjk0In0.mYilNk8xUCvQ20rjdMfBZcTLGbjUREgP_OhA7b-wuws

// postgres://612a2dd0a487491a9b401ca7ed535fbb5211055040111e2935a59fc4f9bc9470:sk_RHPsCwIffMVj_2Z6Yfwha@db.prisma.io:5432/?sslmode=require
...
app.put('/comments/:id', authenticateToken, async (req, res) => {
  ...
  if (!content) {
    return res.status(400).json({ message: 'New comment content is required.' });
  }
  ...
  try {
    ...
    if (timeElapsed > GRACE_PERIOD_MINUTES) {
      return res.status(403).json({ message: `Comments can only be edited within ${GRACE_PERIOD_MINUTES} minutes.` });
    }
...
app.put('/comments/:id/restore', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (comment.user_id !== userId) {
      return res.status(403).json({ message: 'You are not authorized to restore this comment.' });
    }
